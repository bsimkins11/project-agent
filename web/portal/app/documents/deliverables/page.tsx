...
export default function DeliverableDocuments() {
  ...
  const loadDocuments = async () => {
    setLoading(true)
    try {
      const response = await getDocumentsByCategory('deliverable')
      setDocuments(response.documents)
    } catch (error) {
      console.error('Failed to load deliverable documents:', error)
    } finally {
      setLoading(false)
    }
  }
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Approved project deliverables, reports, and final outputs.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <div className="text-gray-500 mb-2">No deliverable documents available</div>
