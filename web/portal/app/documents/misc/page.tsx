...
export default function MiscDocuments() {
  ...
  const loadDocuments = async () => {
    setLoading(true)
    try {
      const response = await getDocumentsByCategory('misc')
      setDocuments(response.documents)
    } catch (error) {
      console.error('Failed to load misc documents:', error)
    } finally {
      setLoading(false)
    }
  }
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Other approved project documents, notes, and reference materials.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <div className="text-gray-500 mb-2">No miscellaneous documents available</div>
