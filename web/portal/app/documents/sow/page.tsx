...
export default function SOWDocuments() {
  ...
  const loadDocuments = async () => {
    setLoading(true)
    try {
      const response = await getDocumentsByCategory('sow')
      setDocuments(response.documents)
    } catch (error) {
      console.error('Failed to load SOW documents:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDownload = (doc: Document) => {
    // For demo purposes, we'll open the document in a new tab
    // In production, this would trigger a proper download
    const link = doc.web_view_link || doc.webViewLink
    if (link) {
      window.open(link, '_blank')
    }
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Approved project statements of work and related documents.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <div className="text-gray-500 mb-2">No SOW documents available</div>
